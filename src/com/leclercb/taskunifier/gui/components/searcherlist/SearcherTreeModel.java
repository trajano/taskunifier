...
public class SearcherTreeModel extends DefaultTreeModel implements ListChangeListener, PropertyChangeListener {
	...
	static {
		...
		GENERAL_TASK_SEARCHERS[0] = new TaskSearcher(Translations.getString("searcherlist.general.all_tasks"),
		...
		GENERAL_TASK_SEARCHERS[1] = new TaskSearcher(Translations.getString("searcherlist.general.hot_list"),
		...
		GENERAL_TASK_SEARCHERS[2] = new TaskSearcher(Translations.getString("searcherlist.general.starred"),
		...
		GENERAL_TASK_SEARCHERS[3] = new TaskSearcher(Translations.getString("searcherlist.general.next_action"),
		...
		GENERAL_TASK_SEARCHERS[4] = new TaskSearcher(Translations.getString("searcherlist.general.completed"),
	...
	public TreePath getDefaultTaskSearcherPath() {
		return new TreePath(this.getPathToRoot(this.categoryGeneral.getChildAt(0)));
	}

	private void initializeGeneralCategory() {
		this.categoryGeneral = new CategoryTreeNode("searcher.category.general.expanded",
	...
	private void initializeContextCategory() {
		this.categoryContext = new CategoryTreeNode("searcher.category.context.expanded",
	...
	private void initializeFolderCategory() {
		this.categoryFolder = new CategoryTreeNode("searcher.category.folder.expanded",
	...
	private void initializeGoalCategory() {
		this.categoryGoal = new CategoryTreeNode("searcher.category.goal.expanded",
	...
	private void initializeLocationCategory() {
		this.categoryLocation = new CategoryTreeNode("searcher.category.location.expanded",
	...
	private void initializePersonalCategory() {
		this.categoryPersonal = new CategoryTreeNode("searcher.category.personal.expanded",
	...
	private MutableTreeNode getTreeNodeFromUserObject(Object userObject) {
		return this.getTreeNodeFromUserObject((DefaultMutableTreeNode) this.getRoot(), userObject);
	}

	private MutableTreeNode getTreeNodeFromUserObject(DefaultMutableTreeNode parent, Object userObject) {
		for (int i = 0; i < parent.getChildCount(); i++) {
			if (EqualsUtils.equals(((DefaultMutableTreeNode) parent.getChildAt(i)).getUserObject(), userObject))
				return (MutableTreeNode) parent.getChildAt(i);

			if (!parent.getChildAt(i).isLeaf()) {
				MutableTreeNode node = this.getTreeNodeFromUserObject((DefaultMutableTreeNode) parent.getChildAt(i),
						userObject);
	...
	public void listChange(ListChangeEvent event) {
		if (event.getValue() instanceof Model) {
			...
			if (event.getChangeType() == ListChangeEvent.VALUE_ADDED) {
				this.insertNodeInto(new ModelTreeNode((Model) event.getValue()), node, node.getChildCount() - 1);
		...
		if (event.getValue() instanceof TaskSearcher) {
			if (event.getChangeType() == ListChangeEvent.VALUE_ADDED) {
				this.insertNodeInto(new SearcherTreeNode((TaskSearcher) event.getValue()),
						this.categoryPersonal,
						this.categoryPersonal.getChildCount() - 1);
			} else if (event.getChangeType() == ListChangeEvent.VALUE_REMOVED) {
				MutableTreeNode child = this.getTreeNodeFromUserObject(event.getValue());
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (event.getSource() instanceof Model) {
			...
			if (!((Model) event.getSource()).getModelStatus().equals(ModelStatus.LOADED)
				...
				int index = node.getIndex(new ModelTreeNode((Model) event.getSource()));

				if (index == -1)
					this.insertNodeInto(new ModelTreeNode((Model) event.getSource()), node, node.getChildCount() - 1);
				else
					this.nodeChanged(new ModelTreeNode((Model) event.getSource()));
			}
		}
		...
		if (event.getSource() instanceof TaskSearcher) {
			if (event.getPropertyName().equals(TaskSearcher.PROP_TITLE)
				...
				this.nodeChanged(new SearcherTreeNode((TaskSearcher) event.getSource()));
			}
		}
	}

		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent event) {
				...
				if (event.isPopupTrigger() || event.getButton() == MouseEvent.BUTTON3) {
					TreePath path = TaskFilterTree.this.getPathForLocation(event.getX(), event.getY());
					...
					if (node instanceof TaskFilterTreeNode) {
						...
						JRadioButtonMenuItem itemAnd = new JRadioButtonMenuItem(TranslationsUtils
								.translateTaskFilterLink(Link.AND));
						JRadioButtonMenuItem itemOr = new JRadioButtonMenuItem(TranslationsUtils

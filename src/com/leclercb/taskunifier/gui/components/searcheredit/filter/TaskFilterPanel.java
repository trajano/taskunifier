...
public class TaskFilterPanel extends JPanel {
	...
	private void initializeButtons(JPanel buttonsPanel) {
			...
			public void actionPerformed(ActionEvent event) {
				if (event.getActionCommand().startsWith("ADD")) {
					...
					if (event.getActionCommand().equals("ADD_ELEMENT")) {
						TaskFilterElement element = new TaskFilterElement(TaskColumn.TITLE, StringCondition.EQUALS, "");
					...
					if (node instanceof TaskFilterTreeNode) {
						((TaskFilterTreeNode) node).getFilter().getParent()
								.removeFilter(((TaskFilterTreeNode) node).getFilter());
					} else if (node instanceof TaskFilterElementTreeNode) {
						((TaskFilterElementTreeNode) node).getElement().getParent()
								.removeElement(((TaskFilterElementTreeNode) node).getElement());
					}
				}
			}
		...
		this.addElementButton = new JButton(Translations.getString("searcheredit.add_element"),
		...
		this.addFilterButton = new JButton(Translations.getString("searcheredit.add_filter"),
				Images.getResourceImage("add.png", 16, 16));

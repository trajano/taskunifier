...
public class ToodledoConfigurationPanel extends ConfigurationPanel {
	...
	public void saveAndApplyConfig() {
		...
		if (!EqualsUtils.equals(Settings.getStringProperty("synchronizer.keep_tasks_completed_for_x_days"),
				this.getValue("KEEP")))
	...
	private void initialize() {
		...
		if (Settings.getEnumProperty("synchronizer.choice", SynchronizerChoice.class) != null)
			toodledoChoiceValue = (SynchronizerChoice) Settings.getEnumProperty("synchronizer.choice",
					SynchronizerChoice.class);
		...
		this.addField(new ConfigurationField("EMAIL", Translations.getString("configuration.toodledo.email"),
				new ConfigurationFieldType.TextField(toodledoEmailValue)));

		this.addField(new ConfigurationField("PASSWORD", Translations.getString("configuration.toodledo.password"),
				new ConfigurationFieldType.PasswordField(toodledoPasswordValue)));

		ConfigurationFieldType.ComboBox comboBox = new ConfigurationFieldType.ComboBox(SynchronizerChoice.values(),
				toodledoChoiceValue);

		comboBox.setRenderer(new SynchronizerChoiceListCellRenderer());

		this.addField(new ConfigurationField("CHOICE", Translations.getString("configuration.toodledo.choice"),
				comboBox));

		this.addField(new ConfigurationField("KEEP", Translations.getString("configuration.toodledo.keep_tasks_for"),
		...
		this.addField(new ConfigurationField("CREATE_ACCOUNT_LABEL", null, new ConfigurationFieldType.Label(
				Translations.getString("configuration.toodledo.create_account"))));

		this.addField(new ConfigurationField("CREATE_ACCOUNT", null, new ConfigurationFieldType.Button(
				new ActionCreateAccount() {
		...
		this.addField(new ConfigurationField("SYNCHRONIZE_ALL_LABEL", null, new ConfigurationFieldType.Label(
				Translations.getString("configuration.toodledo.synchronize_all"))));

		this.addField(new ConfigurationField("SYNCHRONIZE_ALL", null, new ConfigurationFieldType.Button(
				new ActionSynchronize() {
		...
		this.addField(new ConfigurationField("RESET_ALL_LABEL", null, new ConfigurationFieldType.Label(Translations
				.getString("configuration.toodledo.reset_all"))));

		this.addField(new ConfigurationField("RESET_ALL", null, new ConfigurationFieldType.Button(
				new ActionSynchronize() {

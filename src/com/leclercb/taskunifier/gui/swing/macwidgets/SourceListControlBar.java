 ...
 * {@link SourceList}. This control bar is displayed at the base of the
 * {@code SourceList}. The control bar also has a draggable widget that can
 * control the divider location of a given {@link JSplitPane}.
 * </p>
 * <p>
 ...
 * </pre>
 * 
 * The above code creates a control bar that looks like this: <br>
 * <br>
 * <img src="../../../../graphics/SourceListControlBar-empty.png">
 * </p>
 * <p>
 * The following code adds two push buttons and a drop-down button to the
 * control bar:
 * 
 * <pre>
...
public class SourceListControlBar {
	...
	private static final ImageIcon SPLITTER_HANDLE = new ImageIcon(
			SourceListControlBar.class.getResource("/com/explodingpixels/macwidgets/images/splitter_handle.png"));
	...
	private void init() {
		...
		this.fSplitterHandle.setCursor(Cursor.getPredefinedCursor(Cursor.W_RESIZE_CURSOR));
	}
	
	/**
	 * Connects the draggable widget in this {@code SourceListControlBar} to the
	 * divider of the
	 * given {@link JSplitPane}. Thus when the user drags the
	 * {@code SourceListControlBar} draggable
	 * widget, the given {@code JSplitPane}s divider location will be adjusted.
	 * 
	 * @param splitPane
	 *            the {@code JSplitPane} to connect the draggable widget to.
	 */
	 ...
	 * {@code SourceListControlBar}. The
	 * returned {@link JComponent} should be added to a container that will be
	 * displayed.
	 * 
	 * @return the user interface component representing this
	 *         {@code SourceListControlBar}.
	 */
	...
	/**
	 * Add a new pop-down style button. The given {@link PopupMenuCustomizer}
	 * will be called just
	 * prior to each showing of the menu.
	 * 
	 * @param icon
	 *            the icon to use in the pop-down menu.
	 * @param popupMenuCustomizer
	 *            the {@code PopupMenuCustomizer} to be called just prior to
	 *            showing
	 *            the menu.
	 */
	public void createAndAddPopdownButton(
			Icon icon,
	...
	/**
	 * Adds a new button with the given icon. The given {@link ActionListener}
	 * will be called when
	...
	public void createAndAddButton(Icon icon, ActionListener actionListener) {
		JComponent button = MacButtonFactory.createGradientButton(
				icon,
				actionListener);
	...
	public void createAndAddButton(Action action) {
		JComponent button = MacButtonFactory.createGradientButton(action);
		initSourceListButton(button);
		this.addComponent(button);
	}
	...
	private class SplitterHandleMouseMovementHandler extends MouseAdapter implements MouseMotionListener {
		...
		public void mousePressed(MouseEvent e) {
			MouseEvent convertedEvent = SwingUtilities.convertMouseEvent(
					SourceListControlBar.this.fSplitterHandle,
					e,
					SourceListControlBar.this.fSplitPane);
			
			this.fDelta = SourceListControlBar.this.fSplitPane.getDividerLocation()
					- convertedEvent.getPoint().x;
		}
		...
		public void mouseDragged(MouseEvent e) {
			...
			// bound newLocation between the minimum and maximum divider
			// locations.
			int boundedNewLocation = Math.max(
					SourceListControlBar.this.fSplitPane.getMinimumDividerLocation(),
					Math.min(
							newLocation,
							SourceListControlBar.this.fSplitPane.getMaximumDividerLocation()));

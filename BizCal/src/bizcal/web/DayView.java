...
public class DayView extends CalendarView {
	...
	public Element getContent() throws Exception {
		...
		while (i.hasNext()) {
			...
			if (image == null)
				...
				Img img = new Img();
				img.setSrc("image?table=resource&column=image&key="
						+ cal.getId());
				th = new TH(img);
			...
			trCal.addElement(th);
			List events = this.getModel().getEvents(cal.getId());
			...
			for (int j = 0; j < this.getDayCount(); j++) {
				...
				eventMatrix.put(
						new Tuple(cal.getId(), DateUtil.round2Day(date)),
						slots);
		...
		for (long pos = startTime.getValue(); pos < endTime.getValue(); pos += resolution) {
			tr = new TR();
			table.addElement(tr);
			...
			while (j.hasNext()) {
				...
				Date date = this.getInterval().getStartDate();
				for (int k = 0; k < this.getDayCount(); k++) {
					...
					if (slot != null) {
						if (!NullSafe.equals(slot, prevSlot)
								|| slot.isBackground())
							tr.addElement(this.createCell(
									slot,
									interval,
									cal,
									time));
					} else
						tr.addElement(this.createFreeCell(cal, time));
					date = DateUtil.getDiffDay(date, +1);
				}
			}
			slotno++;
	...
	private DateInterval getInterval(Date date) throws Exception {
		return new DateInterval(
				this.getModel().getViewStart().getDate(date),
				this.getModel().getViewEnd().getDate(date));
	}

...
public class EventPopupMenu extends JPopupMenu {
	...
	public EventPopupMenu(
			Object calId,
			Event event,
			CalendarListener calListener) throws Exception {
		...
		public void actionPerformed(ActionEvent event) {
			try {
				if ("copy".equals(event.getActionCommand())) {
					this._listener.copy(Collections.nCopies(
							1,
							EventPopupMenu.this._event));
					return;
				}
				...
				if ("properties".equals(event.getActionCommand())) {
					this._listener.showEvent(
							EventPopupMenu.this._calId,
							EventPopupMenu.this._event);
				}
			} catch (Exception e) {

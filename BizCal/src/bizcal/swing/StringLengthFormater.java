...
public class StringLengthFormater {
	...
	public static String formatDateString(
			...
			Font aFont,
			int aWidth,
			ArrayList aPatternList) throws Exception {
		...
		while (!fontFits) {
			if (aPatternList.size() > i) {
				format = new SimpleDateFormat(
						aPatternList.get(i).toString(),
						Locale.getDefault());
				string = TextUtil.formatCase(format.format(aDate));
			} else {
				string = TextUtil.formatCase(format.format(aDate)).substring(
						0,
						1);
				fontFits = true;
			}
	...
	public static String getCommonDateFormat(
		...
		while (iter.hasNext()) {
			...
			while (!fontFits) {
				if (aPatternList.size() > i) {
					format = new SimpleDateFormat(
							(String) aPatternList.get(i),
							Locale.getDefault());
					string = TextUtil.formatCase(format.format(date));
				} else {
					string = TextUtil.formatCase(format.format(date)).substring(
							0,
							1);
					fontFits = true;
				}

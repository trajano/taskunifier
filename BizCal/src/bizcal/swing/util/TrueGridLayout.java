...
public class TrueGridLayout implements LayoutManager {
	...
	public Dimension preferredLayoutSize(Container parent) {
		...
		return new Dimension(maxWidth * (int) this.cols, maxHeight
				* (int) this.rows);
	}
	...
	public void layoutContainer(Container parent) {
		double row = 0;
		double col = 0;
		double width = parent.getWidth() / this.cols;
		double height = parent.getHeight() / this.rows;
		for (int i = 0; i < parent.getComponents().length; i++) {
			Component child = parent.getComponents()[i];
			double x1 = col * width;
			...
			col++;
			// if (col == this.cols) {
			if (Math.abs(col - this.cols) < 0.0000001) {
				col = 0;
				row++;
			}
		}
